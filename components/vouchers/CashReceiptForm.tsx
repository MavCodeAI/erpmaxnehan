...
import { CashReceiptVoucher, Account, VoucherEntry, AllTransactions } from '../../types';
import { Plus, Trash2 } from 'lucide-react';
import { getAccountBalance } from '../../utils/calculations';

interface CashReceiptFormProps {
  voucher: CashReceiptVoucher | null;
...
const CashReceiptForm: React.FC<CashReceiptFormProps> = ({ voucher, onSave, onCancel, lastVoucherNumber, accounts, allTransactions }) => {
     ...
     const handleEntryChange = (index: number, field: keyof EntryRow, value: string | number) => {
        ...
        if (field === 'accountId') {
            ...
            if (accountId) {
                ...
                currentEntry.closingBalance = balance - currentEntry.amount; // Receipt CREDITS the account
            } else {
            ...
            currentEntry.closingBalance = currentEntry.currentBalance - amount; // Receipt CREDITS the account
        } else {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const newVoucher: CashReceiptVoucher = {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg">
        <div className="p-6 border-b dark:border-gray-700">
             <h3 className="text-2xl font-semibold">{isEditMode ? `Edit Cash Receipt #${voucher?.voucherNumber}` : 'New Cash Receipt'}</h3>
        </div>
      <form onSubmit={handleSubmit} className="p-6">
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <label className={labelClasses}>Received In</label>
        ...
        <div className="flex justify-end p-6 space-x-4 border-t dark:border-gray-700 mt-6">
          ...
          <Button type="submit" variant="primary" disabled={totalAmount === 0}>Save Receipt</Button>
        </div>
      </form>
    </div>
  );

...
import { PlusCircle, FileDown, Search, Edit, Trash2, ChevronDown, Printer, Receipt, Eye, Calendar, AlertCircle } from 'lucide-react';
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border-l-4" style={{ borderLeftColor: color }}>
        <div className="flex items-center justify-between">
            <div className="flex-1">
                ...
                {count !== undefined && (
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{count} bill{count !== 1 ? 's' : ''}</p>
                )}
            </div>
...
const BillsListPage: React.FC<BillsListPageProps> = ({ bills, onDeleteBill }) => {
    ...
    const filteredBills = useMemo(() => {
        ...
        return bills.filter(bill => {
            ...
            const vendorName = (bill.vendorName || '').toString().toLowerCase();
            
            return billNumber.includes(safeSearchTerm) || vendorName.includes(safeSearchTerm);
    ...
    return (
        <>
            <div className="flex justify-between items-center my-6">
                <div>
                    <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100">Bills</h2>
                    <p className="text-gray-600 dark:text-gray-400 mt-1">Manage your purchase bills and track payables</p>
                </div>
            ...
            <div className="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <SummaryCard title="All Bills" amount={summaryData.total} count={summaryData.totalCount} icon={Receipt} color="#3b82f6" />
                        ...
                        <tbody className="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                           {paginatedBills.length === 0 && filteredBills.length === 0 ? (
                               ...
                               <tr key={bill.id}>
                                   ...
                                   <td className="px-6 py-4 whitespace-nowrap font-medium text-blue-600 dark:text-blue-400">{bill.billNumber}</td>
                                   ...
                                   <td className="px-6 py-4 whitespace-nowrap text-right">
                                        <div className="flex items-center justify-end space-x-2">
                                            <Button size="sm" variant="secondary" icon={Eye} onClick={() => navigate(`/purchases/bills/${bill.id}`)} title="View" />
                                            <Button size="sm" variant="secondary" icon={Edit} onClick={() => navigate(`/purchases/bills/${bill.id}`)} title="Edit" />
                                            <Button size="sm" variant="danger" icon={Trash2} onClick={() => setBillToDelete(bill)} title="Delete" />
                                        </div>
                                   </td>
                               </tr>
                           ))}

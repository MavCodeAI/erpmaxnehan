...
import { BankReceiptVoucher, Account, AllTransactions } from '../types';
import TransactionPageLayout from '../components/layout/TransactionPageLayout';
import TransactionList from '../components/ui/TransactionList';
import BankReceiptForm from '../components/vouchers/BankReceiptForm';
import Modal from '../components/ui/Modal';
import Button from '../components/ui/Button';

interface BankReceiptsProps {
    vouchers: BankReceiptVoucher[];
...
const BankReceipts: React.FC<BankReceiptsProps> = ({ vouchers, onSave, onDelete, accounts, allTransactions }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedVoucher, setSelectedVoucher] = useState<BankReceiptVoucher | null>(null);
    const [isCreating, setIsCreating] = useState(false);
    const [voucherToDelete, setVoucherToDelete] = useState<BankReceiptVoucher | null>(null);

    const filteredVouchers = vouchers.filter(voucher =>
        voucher.voucherNumber.toLowerCase().includes(searchTerm.toLowerCase())
    ).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    
    const handleSelectItem = (voucher: BankReceiptVoucher) => {
        setIsCreating(false);
    ...
    const handleSave = (voucher: BankReceiptVoucher) => {
        onSave(voucher);
        handleCancel();
    };
    ...
    return (
        <>
            <TransactionPageLayout
                ...
                list={
                    <TransactionList<BankReceiptVoucher>
                ...
                form={
                    (isCreating || selectedVoucher) && (
                        <BankReceiptForm
                            ...
                            lastVoucherNumber={vouchers[0]?.voucherNumber || 'BR-2023-000'}
                            accounts={accounts}
                            allTransactions={allTransactions}
                        />
                    )
